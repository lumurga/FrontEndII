...
function renderizarDatosUsuario(datos) {
      let cajaContenedora = document.querySelector(".tarjeta")
      let datosParseados = JSON.parse(datos).results
      for(dato of datosParseados){
        cajaContenedora.innerHTML += `
        <h2>${dato.name.title} ${dato.name.first} ${dato.name.last} </h2>
        <p>${dato.email} </p>
...
renderizarDatosUsuario(baseDeDatos);